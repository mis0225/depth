...
def recognize_from_image(net):
    ...
    for image_path in args.input:
        ...
        depth_values = result[0]  # Assuming the depth result is the first element

        # Create a colormap for visualization
        colormap = cv2.applyColorMap(
            cv2.convertScaleAbs(depth_values, alpha=0.03), cv2.COLORMAP_JET
        )

        # Conver colormap to float32
        colormap_float32 = colormap.astype("float32")

        # Overlay colormap on the original image
        overlaid_img = cv2.addWeighted(img[0].transpose(1, 2, 0).astype("float32"), 0.7, colormap_float32, 0.3, 0, dtype=cv2.CV_32F)


        # Display and save the overlaid image
        cv2.imshow("Depth Visualization", overlaid_img)
        cv2.imwrite("depth_visualization.png", overlaid_img)

        if cv2.waitKey(0) & 0xFF == ord('q'):
            break

        # Save depth values to a CSV file
        csv_file_path = r"C:\Users\420204\ailia-models\depth_estimation\midas\output\depth_values.csv"
...
def recognize_from_video(net):
    ...
    while True:
        ...
        # convert depth result to 8-bit image
        res_img = result_to_image(result)

        # create obstacle mask
        depth_values = result[0]
        depth_threshold = 1000
        obstacle_mask = depth_values < depth_threshold

        # create overlay image
        overlay = create_overlay(frame, obstacle_mask)

        # combine images
        combined_frame = np.hstack((res_img, overlay))
        
        cv2.imshow('depth', combined_frame)
        frame_shown = True
...
def result_to_image(result):
    ...
    if depth_max - depth_min > np.finfo("float").eps:
        ...
        out = 0
    res_img = (out / 256).astype("uint8")
    return cv2.cvtColor(res_img, cv2.COLOR_GRAY2BGR)

def create_overlay(frame, obstacle_mask):
    overlay = frame.copy()
    ...
    overlaid_frame = cv2.addWeighted(frame, 0.7, overlay, 0.3, 0)
